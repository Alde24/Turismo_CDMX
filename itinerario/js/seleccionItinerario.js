...
$(document).ready(() => {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if(response.LugaresBaseDatos){
                //Asignar a la variable los datos obtenidos por la consulta
                lugaresBase = response.LugaresBaseDatos;
                console.log(lugaresBase);
                fotosBase = response.Fotos;
    ...
    if(ConoceLugares == 'Si'){
        /* Inicia lógica de comida */
$("#BtnSeccComida").on('click', function() {
    // Ocultamos la sección de categorías de usuario
    ...
    // Guardar en la variable global la categoría
    ...
    function Comida(fechaSeleccionada) {
        ...
        $.ajax({
            ...
            success: function(responseComida) {
                if (responseComida.status === "success") {
                    ...
                    // Llamar a la función que busca restaurantes cerca
                    si_buscarRestaurantesCerca(destinos, tiposEstablecimientos, resultadosComida).then(() => {
                        ...
                        // Filtrar los primeros 10 resultados por categoría (ajustar según lo necesites)
                        mostrarResultadosComida(resultadosComida);
                    });
        ...
        /* Función para mostrar los primeros 10 resultados */
        function mostrarResultadosComida(resultadosComida) {
            const contenedorTarjetas = document.getElementById('tab-content'); // Contenedor para las cards
            contenedorTarjetas.innerHTML = ""; // Limpiar el contenedor antes de agregar nuevas tarjetas

            // Iterar por cada categoría en los resultados
            for (const categoria in resultadosComida) {
                const establecimientos = resultadosComida[categoria];

                // Mostrar solo los primeros 10 establecimientos por categoría
                const primerosDiez = establecimientos.slice(0, 10);
                ...
                primerosDiez.forEach(establecimiento => {
                    const detallesComida = {
                        ...
                        descripcion: establecimiento.descripcion || "No disponible", // Valor por defecto si no hay descripción
                        categorias: establecimiento.categorias,
                        precio: establecimiento.precio || "No disponible", // Valor por defecto si no hay precio
                        Latitud: establecimiento.Latitud,
                        Longitud: establecimiento.Longitud,
                    };

                    // Crear la tarjeta con la función que ya tienes
                    const card = crearCardComida(detallesComida);
                    contenedorTarjetas.appendChild(card); // Añadir la tarjeta al contenedor
                });
            }
        }
    ...
    */
    ...
    async function buscarLugaresPorCategoria(categorias) {
        ...
        for (const categoria of categorias) {
            ...
            try {
                //Buscar los lugares de acuerdo al valor de la categoria
                const lugaresFiltrados = lugaresBase.filter(subcategoria =>{
                    return (
                        subcategoria.valor_sub_categoria == categoria
                    )
                } )

               //Vamos pasar a la variable lugaresPorCategoria los lugares filtrados 
               lugaresPorCategoria[categoria] = lugaresFiltrados; 
   ...
   // Función para crear la tarjeta con la URL de la foto
    function crearTarjetaLugar(detallesLugar) {
        ...
        // Crear un ID único para cada tarjeta
        const cardId = `card-${nombre.replace(/\s+/g, '-').replace(/[\s,{}().|""]/g, "-")}`;  // Reemplazamos espacios con guiones y eliminamos comas
    ...
    async function mostrarInformacionLugar(detalles) {
        ...
        foto = fotosBase.filter(f=>{
            return(
                f.lugar_id == detalles.id
            )
        });
        ...
        $('#Descripcion').text(`Descripción: ${desc.descripcion_corta || 'Sin descripción'}`); // Descripción
         $('#Categorias').text(`Categorías: ${categorias ? categorias.join(', ') : 'Sin categorías'}`); // Categorías
         $("#ModalImagen").attr("src", rutaFoto).removeClass("d-none");
    ...
    function Lugares(categoriaSeleccionada){
        ...
        if (categoriasLugaresDistribuidos[categoriaSeleccionada]) {
            ...
            const tabContent = document.getElementById('tab-content');
            tabContent.innerHTML='';      // Limpiar el contenedor de las tarjetas antes de agregar nuevas
            ...
            if (lugaresDelDia && Array.isArray(lugaresDelDia)) {
                ...
                lugaresDelDia.forEach(lugar => {
                    ...
                    // Usar .then() para manejar la promesa
                     ...
                     // Esto elimina todas las tarjetas previas
                    ...
                    const detallesLugar = {
                        ...
                        direccion : lugar.direccion,
                        valoracion: lugar.rating,
                        categoria: lugar.sub_categoria_nombre,
